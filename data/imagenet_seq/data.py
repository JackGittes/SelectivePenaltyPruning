    ...
    def __iter__(self):
        for x, y in self.ds.__iter__():
            if self.cuda:
                ...
                yield uint8_to_float(x, self._mean, self._std), y.long()
            else:
                yield normalize(torch.ByteTensor(x)), torch.IntTensor(y).long()

    def __len__(self):
        return self.ds.size()


def uint8_to_float(x, _mean, _std):
    x = x.permute(0, 3, 1, 2) # pytorch is (n,c,w,h)
    return (x.float() / 255.0 - _mean[None, :, None, None]) / _std[None, :, None, None]
